...
{
    public partial class SalesByPrice : Form
    {
        public SalesByPrice()
        {
            InitializeComponent();
        ...
        private void button_ShowClick(object sender, EventArgs e)
        {
            try
            {
                ...
                myCommand = new SqlCommand("SELECT s.sale_code AS 'Sale Code', p.stock AS 'Stock Name', c.name AS 'Client', s.sale_date AS 'Sale Date', s.quantity AS 'Quantity', s.price_VAT AS 'Price' " +
                    ...
                    "WHERE s.price_VAT > @price AND s.stock_code = p.code AND s.client_id = c.client_id", myConnection);
                myConnection.Open();
        ...
        private void button_SaveToTxtClick(object sender, EventArgs e)
        {
            ...
            string filePath = "D:\\ТУ-Варна\\Семестър 8\\Информационни системи\\Sugar Factory\\Sugar Factory\\queires\\Sales By Price.txt";
